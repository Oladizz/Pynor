...
export const AdminPage: React.FC<{ onNavigate: () => void }> = ({ onNavigate }) => {
    ...
    useEffect(() => {
        const fetchAdminData = async () => {
            ...
            try {
                ...
                const fetchedPings = await getAllPingResults();
        ...
        fetchAdminData();
    }, [getAllUsers, getAllPingResults]);

    // Analytics now use the state variables
    const stats = useMemo(() => {
        return {
            totalUsers: users.length,
            activePings: users.reduce((acc, u) => acc + u.pingedSites.length, 0),
            totalPingEvents: pings.length,
            avgLatency: pings.length > 0 
                ? Math.round(pings.reduce((acc, p) => acc + (p.responseTime || 0), 0) / pings.length) + 'ms' 
                : 'N/A'
        };
    }, [users, pings]);
    ...
    const sitePopularity = useMemo(() => {
        ...
        users.forEach(u => {
            u.pingedSites.forEach(site => {
                counts[site.url] = (counts[site.url] || 0) + 1; // Corrected to use site.url
            });
        });
    ...
    const handleDeleteUser = (id: string, email: string) => {
        if (window.confirm(`Are you sure you want to delete user ${email}? This action cannot be undone.`)) {
            deleteUser(id);
            // After deletion, re-fetch data to update the list
            // For now, simple re-fetch, could be optimized later
            ...
            <main className="flex-grow p-4 md:p-8 overflow-y-auto max-h-screen">
                ...
                {dataLoading && (
                    <div className="flex flex-col items-center justify-center h-64">
                        ...
                        <p className="mt-4 text-text-secondary">Loading admin data...</p>
                    </div>
                )}

                {dataError && (
                    <div className="text-red-400 p-4 bg-red-400/10 rounded-lg flex items-center gap-2">
